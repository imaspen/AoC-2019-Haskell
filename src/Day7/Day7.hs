...
part1 :: Memory -> Int
part1 memory = maximum $ map (run memory 0) combinations

run :: Memory -> Int -> [Int] -> Int
run memory i [] = i
run memory i (input : rest) = run memory result rest
    where (_, result : _) = runProgram (memory, 0) ([input, i], [])
...
combinations :: [[Int]]
combinations = filter ((5==) . length . nub) $ mapM (const [0..4]) [1..5]
